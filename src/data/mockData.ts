 ...
 * concrete date in the *current* week (Mon-Sun containing today) fresh on every call, so the
 * timetable rolls forward automatically instead of staying pinned to one fixed week. Kept in
 * lockstep with the backend's WeekDates.forDay(). */
export function getWeekDates(): Record<Day, string> {
  const today = new Date()
  const isoDayIndex = today.getDay() === 0 ? 7 : today.getDay() // Mon=1..Sun=7
  const monday = new Date(today)
  monday.setDate(today.getDate() - (isoDayIndex - 1))
